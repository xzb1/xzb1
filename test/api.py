...
def send_user(keyword):
    ...
    touser = 'olNUf5v1Ccbl5jr6FaQaPFkqBNGs'
    payload = json.dumps({
        "touser": touser,
...
def send_lili(keyword):
    yiyan = requests.get(url='https://api.lovelive.tools/api/SweetNothings').text
    access_token = get_token()
    url = f"https://api.weixin.qq.com/cgi-bin/message/template/send?access_token={access_token}"
    template_id = 'x80dmd4J2PHe-83ysz0MZoAg3C2hGmpy9Eq2MSeARRI'
    touser = 'olNUf5rFSQnQyPh23LyEc5QnWppI'
    payload = json.dumps({
        "touser": touser,
        "template_id": template_id,
        "data": {
...
if __name__ == "__main__":
    ...
    send_lili(weather(city_id=city_id, day=day))
    send_user(weather(city_id=city_id, day=day))
